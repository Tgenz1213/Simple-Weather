...
export function ThreeDayForecast({ data }: { data: unknown }) {
  ...
  const periods = (data as unknown as { properties?: { periods?: ForecastPeriod[] } })?.properties?.periods ?? [];

  // Prefer daytime periods (isDaytime true) and fall back to first periods
  const daytime = periods.filter((p: ForecastPeriod) => p.isDaytime).slice(0, 3);
  const items = (daytime.length >= 3 ? daytime : periods.slice(0, 3)) as ForecastPeriod[];
    ...
    <section aria-label="3-day-forecast" className="mt-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {items.map((p) => (
          ...
          >
            <div className="text-sm text-slate-500 mb-1">{formatDate(p.startTime)}</div>
            ...
            <div className="text-xs text-slate-500">{p.windSpeed} {p.windDirection}</div>
          </article>
        ))}
      </div>
    </section>
